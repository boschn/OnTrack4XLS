...
Public Class ThisAddIn
  ...
  <ormChangeLogEntry(Application:=ConstApplicationExcelAddin, module:="", version:=1, release:=1, patch:=2, changeimplno:=1, _
            ...
            Try
                ...
                If Not host Is Nothing Then
                    If _CurrentHost Is Nothing OrElse _CurrentHost.Name <> host.name Then
                        ...
                        ''' 
                        Call CoreMessageHandler(message:=" Host Application switched to " & host.name, argument:=host.path, _
                                                     messagetype:=otCoreMessageType.InternalInfo, procedure:="ThisAddin.CurrentHost")
    ...
    Private Sub ThisAddIn_Startup() Handles Me.Startup
        ...
        '** register the UI
        UserInterface.RegisterNativeUI(UserInterface.LoginFormName, GetType(UI.UIFormLogin))
        ...
        Call CoreMessageHandler(showmsgbox:=False, message:="On Track Excel AddIn Startup", _
                             noOtdbAvailable:=True, procedure:="OTDB.Initialize", messagetype:=otCoreMessageType.InternalInfo)

        '** try to set the current host
        Me.SetCurrentHost()

        '''*** About Data
        OnTrack.UI.AboutData.ApplicationName = "OnTrack4XLS - OnTrack Add-in for Excel"
        OnTrack.UI.AboutData.Description = Me.ApplicationDescription
        OnTrack.UI.AboutData.Version = Me.ApplicationVersion
        OnTrack.UI.AboutData.CopyRight = "(C) by sfk engineering services UG"
        OnTrack.UI.AboutData.Company = Me.ApplicationCompany
        OnTrack.UI.AboutData.ProductName = "OnTrack Database Suite"
    ...
    Friend Function SetConfigProperties(Optional force As Boolean = False) As Boolean
        ...
        '        Case Else
        '            Call CoreMessageHandler(showmsgbox:=True, argument:=value, procedure:="ThisAddin.SetConfigProperties", _
        ...
        If Not value Is Nothing Then
            Select Case LCase(value)
                Case ConstCPVDriverSQLServer
                    ' set it
                    SetConfigProperty(name:=ConstCPNDriverName, weight:=20, value:=ConstCPVDriverSQLServer, configsetname:=useConfigSetName)
                    'SetConfigProperty(name:=ConstCPNDBUseseek, weight:=20, value:=True, configsetname:=useConfigSetName)
                Case ConstCPVDriverOleDB
                    ' set it
                    SetConfigProperty(name:=ConstCPNDriverName, weight:=20, value:=ConstCPVDriverOleDB, configsetname:=useConfigSetName)
                    'SetConfigProperty(name:=ConstCPNDBUseseek, weight:=20, value:=False, configsetname:=useConfigSetName)
                Case Else
                    Call CoreMessageHandler(showmsgbox:=True, argument:=value, procedure:="ThisAddin.SetConfigProperties", _
        ...
        End If
        value = GetHostProperty(ConstCPNDriverID, silent:=True)
        If Not value Is Nothing Then
            SetConfigProperty(name:=ConstCPNDriverName, weight:=20, value:=value, configsetname:=useConfigSetName)
        End If
        ' get the UseLogAgent
